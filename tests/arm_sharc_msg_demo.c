...
static int help(void)
{
	...
	printf("\t-i,--remote_core_id\tnumber of remote core id:\
...
static int mcapi_msg_trans(int thNum, struct mcapi_prams * pPrams)
{
	...
	int ret, thNuM = thNum;
	int mode, b_block, timeout, test_round;
	mcapi_domain_t domain;
...
end_test:
	...
	if (pass_num == test_round) {
		printf("Thread [%d] core0 %d rounds mode(%d) demo Test PASSED!!\n",
					thNum, test_round, mode);
		return 0;
	}
	else {
		printf("Thread [%d] core0 only %d rounds mode(%d) test passed, demo Test FAILED!!\n",
					thNum, pass_num, mode);
		return -1;
	}
}
...
int main (int argc, char *argv[])
{
	...
	unsigned int mode = 0;
	unsigned int b_block = 0;
	unsigned int timeout = 10*1000;
	unsigned int test_round = 100;
	unsigned int remote_core = 0;
	const char short_options[] = "hm:t:r:i:";
	const struct option long_options[] = {
		{"help", 0, NULL, 'h'},
		{"mode", 1, NULL, 'm'},
		{"timeout", 1, NULL, 't'},
		{"round", 1, NULL, 'r'},
		{"remote_core_id", 1, NULL, 'i'},
		{NULL, 0, NULL, 0},
	};

	struct mcapi_prams pthPrams[2];
	int ret = 0;
	int sharc_core_bit = 0;
	...
	while (1) {
		...
		case 'r':
			...
			break;
		case 'i':
			remote_core = strtol(optarg, NULL, 0);
			break;
		default:
	...
	switch(remote_core) {
		case 0:
			sharc_core_bit |= (SHARC_CORE0 | SHARC_CORE1);
			break;
		case 1:
			sharc_core_bit |= (SHARC_CORE0);
			break;
		case 2:
			sharc_core_bit |= (SHARC_CORE1);
			break;
		default:
			printf("Invalid remote_core value: %d\
				\nIt should be in the range of 0 to 2\n",remote_core);
			wrong(__LINE__);
			return -1;
	}
	if (timeout <= 0) {
	...
	if (sharc_core_bit & SHARC_CORE0) {
		...
		pthPrams[0].mode		= mode;
		pthPrams[0].b_block		= b_block;
	...
	if (sharc_core_bit & SHARC_CORE1) {
		...
		pthPrams[1].mode		= mode;
		pthPrams[1].b_block		= b_block;
